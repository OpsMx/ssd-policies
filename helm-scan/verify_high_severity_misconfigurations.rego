...
check_if_high_alert_exists = exists_flag {
  high_severities_counter = count(input.HighSeverity)
...
deny[{"alertMsg": msg, "suggestion": sugg, "error": error }]{
  ...
  some i
  rule = input.HighSeverity[i].RuleID
  title = input.HighSeverity[i].Title
  targets = concat(",\n", input.HighSeverity[i].TargetResources)
  resolution = input.HighSeverity[i].Resolution
