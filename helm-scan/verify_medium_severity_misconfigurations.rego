...
check_if_medium_alert_exists = exists_flag {
  medium_severities_counter = count(input.metadata.results.MediumSeverity)
...
deny[{"alertMsg": msg, "suggestion": sugg, "error": error }]{
  ...
  some i
  rule = input.metadata.results.MediumSeverity[i].RuleID
  title = input.metadata.results.MediumSeverity[i].Title
  targets = concat(",\n", input.metadata.results.MediumSeverity[i].TargetResources)
  resolution = input.metadata.results.MediumSeverity[i].Resolution
