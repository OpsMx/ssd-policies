...
webhook_secret_check = response.body.values[_].secret_set
...
deny[{"alertMsg":msg, "suggestions": sugg, "error": error}]{
  webhook_secret_check = false
  msg := sprintf("Webhook Secret Check failed: Secret not enabled for %v/%v repository.", [input.metadata.owner,input.metadata.repository])
  error := ""
  sugg := sprintf("Adhere to the company policy webhook secret not found for %v/%v repository.", [input.metadata.owner,input.metadata.repository])
}
