...
deny[{ "accountName": scan_account, "alertMsg": msg, "alertStatus": "active", "alertTitle": title, "error": "", "exception": "", "fileApi": download_url, "suggestion": sugg, "justification": justification }] {
  some dep in response.body.dependency_issues
  dep.code == "UNAPPROVED_LICENSE_DEP_FILE"
  justification := object.get(dep.details, "license", "")
...
deny[{ "accountName": scan_account, "alertMsg": msg, "alertStatus": "active", "alertTitle": title, "error": "", "exception": "", "fileApi": download_url, "suggestion": sugg, "justification": justification }] {
  ...
  justification := object.get(dep.details, "library", "")
...
deny[{ "accountName": scan_account, "alertMsg": msg, "alertStatus": "active", "alertTitle": title, "severity": dep.details.severity,"error": "", "exception": "", "fileApi": download_url, "suggestion": sugg, "justification": justification }] {
  some dep in response.body.dependency_issues
  dep.code == "VULNERABLE_DEPENDENCY_DEP_FILE"
  upper(dep.details.severity) == upper(policy_severity)
  justification := sprintf("CVE %v (%v)", [object.get(dep.details, "cve", ""), object.get(dep.details, "severity", "")])
