...
issues = [response.body.vulnerabilities[i] | response.body.vulnerabilities[i].risk == "Critical"]
...
deny[{"alertMsg": msg, "suggestion": sugg, "error": error, "exception": "", "alertStatus": alertStatus, "accountName": scan_account}] {
    count_issues == -1
    msg = "List of Critical Severity Issues for PentestGPT Scan could not be accessed."
