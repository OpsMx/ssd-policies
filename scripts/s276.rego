...
deny[{"alertTitle": title, "alertMsg": msg, "suggestion": sugg, "error": error, "exception": "", "alertStatus": alertStatus, "accountName": scan_account}]{
	...
	secret_severity == "MEDIUM"
	secret_ruleid = j.RuleID
	secret_start_line = j.StartLine
	secret_end_line = j.EndLine
	secret_highlight = j.Match	

	title := sprintf("Medium Severity Secret detected in code: %v", [secret_title])
	msg := sprintf("Secret found for %v/%v code repository in branch %v.\nSecret identified: %v, \nRule Violated: %v. \nFileName: %v. \nStartLine: %v. \nEnd Line: %v. \nHighlighted text: %v.", [input.metadata.owner, input.metadata.repository, input.metadata.branch, secret_title, secret_ruleid, secret_file, secret_start_line, secret_end_line, secret_highlight])
